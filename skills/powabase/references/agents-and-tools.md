...
1. **Data — knowledge bases** (link-KB endpoint in §1). Decide *exactly* which KBs the
   ...
   one query? Don't attach — pass `runtime_knowledge_bases` on that run (§2,
...
does ONE up-front retrieval on the raw message and injects it (no tool). Semantics:
...
knob types/ranges, unknown-key typos rejected, `source_ids` must be indexed in that
entry's KB) with a **10-entry cap**; NOT part of the four-way mutual-exclusivity
rule — combines with any context source (with preload `knowledge_bases`, both
retrievals run and bill, no dedup). `max_context_tokens` is honored only when the
run's tool resolves to exactly one KB. On non-streaming `/run` the field 400s (no
